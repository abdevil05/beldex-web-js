...
const JustRegisterTransactionDetailsView = require("./JustRegisterTransactionDetailsView.web");
...
class RegisterFundsView extends View {
  ...
  setup_views() {
    ...
    self._setup_form_containerLayer();
    {
      ...
      if (self._isUsingRelativeNotFixedActionButtonsContainer() === false) {
        ...
        view =
          commonComponents_actionButtons.New_Stacked_ActionButtonsContainerView(
            16,
            16,
            15,
            self.context
  ...
  _setup_form_containerLayer() {
    ...
    self.form_containerLayer = containerLayer;
    {
      ...
      //
      // initial config
      self.context.settingsController.executeWhenBooted(
        // wait for boot for this
        function () {
  ...
  _setup_form_walletSelectLayer() {
    ...
    div.className = "form_field";
    {
      ...
      {
        const tooltipText =
          'Beldex makes transactions<br/>with your "available outputs",<br/>so part of your balance will<br/>be briefly locked and then<br/>returned as change.';
        const view = commonComponents_tooltips.New_TooltipSpawningButtonView(
          tooltipText,
          self.context
        );
  ...
  _setup_form_amountInputLayer(tr) {
    ...
    self.amountInputLayer.addEventListener("keyup", function (event) {
      self.set_isSubmittable_needsUpdate();
      self.set_effectiveAmountLabelLayer_needsUpdate();
    });
    //
    self.ccySelectLayer = pkg.ccySelectLayer;
    self.ccySelectLayer.addEventListener("change", function () {
    ...
    {
      const tooltipText = `Currency selector for<br/>display purposes only.<br/>The app will send ${Currencies.ccySymbolsByCcy.XMR}.<br/><br/>Rate providers include<br/>${rateServiceDomainText}.`;
      const view = commonComponents_tooltips.New_TooltipSpawningButtonView(
        tooltipText,
        self.context
      );
    ...
    const breakingDiv = document.createElement("div");
    {
      ...
      const layer = commonComponents_forms.New_fieldTitle_labelLayer(
        "",
        self.context
      );
    ...
    {
      const tooltipText =
        "Based on Beldex network<br/>fee estimate (not final).<br/><br/>MyBeldex does not charge<br/>a transfer service fee.";
      const view = commonComponents_tooltips.New_TooltipSpawningButtonView(
        tooltipText,
        self.context
      );
  ...
  _new_required_contactPickerLayer() {
    const self = this;
    const layer =
      commonComponents_contactPicker_Lite.New_contactPickerLayer_Lite(
        self.context,
        "Beldex address",
        function (event) {
  ...
  _setup_form_contactOrAddressPickerLayer() {
    ...
    {
      const tooltipText =
        "Drag &amp; drop QR codes<br/>to auto-fill.<br/><br/>Please double-check<br/>your recipient info as<br/>Beldex transfers are<br/>not yet&nbsp;reversible.";
      const view = commonComponents_tooltips.New_TooltipSpawningButtonView(
        tooltipText,
        self.context
      );
    ...
    div.appendChild(layer);
    {
      ...
      if (self.fromContact !== null) {
        setTimeout(
          // must do this on the next tick so that we are already set on the navigation controller
          function () {
            self.contactOrAddressPickerLayer.ContactPicker_pickContact(
              self.fromContact
            ); // simulate user picking the contact
          },
          1
        );
      }
    }
    {
      // 'resolving' act ind
      const layer =
        commonComponents_activityIndicators.New_Resolving_ActivityIndicatorLayer(
          self.context
        );
    ...
    }
    {
      ...
      fieldContainerLayer.style.display = "none"; // initial state
      {
        ...
        detectedMessage.style.fontFamily =
          "Native-Light, input, menlo, monospace";
  ...
  _setup_form_manualPaymentIDInputLayer() {
    ...
    div.style.display = "none"; // initial
    {
      ...
      labelRowContainer.style.margin = "0 0 8px 0";
      {
        ...
        //
        const generateButtonView =
          commonComponents_tables.New_clickableLinkButtonView(
            "GENERATE ONE",
            self.context,
            function () {
              self.manualPaymentIDInputLayer.value =
                self.context.monero_utils.new_payment_id();
            }
          );
  ...
  _handleRegisterButtonClick(inputValue) {
    ...
    if (inputValue) {
      ...
      const valuesArrayFromObject = Object.values(myDynamicObject);
      console.log("Array elements : ", valuesArrayFromObject);

      const self = this;
      self._tryToGenerateRegister(valuesArrayFromObject);
    } else {
  ...
  _setup_newTextInputField(placeholderText) {
    ...
    const valueLayer = commonComponents_forms.New_fieldValue_textInputLayer(
      self.context,
      {
        placeholderText: placeholderText || "Placeholder Text",
      }
    );
  ...
  _setup_form_field_priority() {
    ...
    div.className = "form_field";
    {
      ...
      {
        const tooltipText =
          "Current default will be normal fee .For both Normal and Flash takes same time to confirm in web wallet <br/><br/>Thank you .";
        const view = commonComponents_tooltips.New_TooltipSpawningButtonView(
          tooltipText,
          self.context
        );
      ...
      self.prioritySelectLayer = selectLayer;
      {
        ...
        selectLayer.style.boxShadow =
          "0 0.5px 1px 0 #161416, inset 0 0.5px 0 0 #494749";
      ...
      selectContainerLayer.appendChild(selectLayer);
      {
        ...
        layer.style.backgroundImage =
          "url(./src/assets/img/dropdown-arrow-down@3x.png)"; // borrowing this
  ...
  startObserving() {
    const self = this;
    self.registrantForDeleteEverything_token =
      ...
      self._walletAppCoordinator_fn_EventName_didTrigger_sendFundsToContact =
        function (contact) {
          self.navigationController.DismissModalViewsToView(
            // whether we should force-dismiss these (create new contact) is debatable…
            ...
            function () {
              ...
              self.navigationController.PopToRootView(
                // now pop pushed stack views - essential for the case they're viewing a transaction
                true, // animated
                function (err) {
                  if (self.isSubmitButtonDisabled == true) {
                    console.warn(
                      "Triggered send funds from contact while submit btn disabled. Beep."
                    );
            ...
            function () {
              ...
              self.navigationController.PopToRootView(
                ...
                function (err) {
                  if (self.isSubmitButtonDisabled == true) {
                    console.warn(
                      "Triggered send funds from wallet while submit btn disabled. Beep."
                    );
                    // TODO: create system service for playing beep, an electron (shell.beep) implementation, and call it to beep
                    // TODO: mayyybe alert tx in progress
                    return;
    ...
    }
    {
      ...
      if (
        self._passwordController_EventName_willDeconstructBootedStateAndClearPassword_listenerFn !==
          null &&
        typeof self._passwordController_EventName_willDeconstructBootedStateAndClearPassword_listenerFn !==
          "undefined"
      ) {
        throw Error(
          "self._passwordController_EventName_willDeconstructBootedStateAndClearPassword_listenerFn not nil in " +
  ...
  cancelAny_requestHandle_for_oaResolution() {
    ...
    if (
      typeof self.resolving_activityIndicatorLayer !== "undefined" &&
      self.resolving_activityIndicatorLayer != null
    ) {
  ...
  stopObserving() {
    const self = this;
    self.context.passwordController.RemoveRegistrantForDeleteEverything(
      self.registrantForDeleteEverything_token
    );
  ...
  //
  Navigation_Title() {
    return "Register Beldex";
  }

  Navigation_New_RightBarButtonView() {
    const self = this;
    const view =
      commonComponents_navigationBarButtons.New_RightSide_SaveButtonView(
        self.context
      );
    self.rightBarButtonView = view;
    const layer = view.layer;
    layer.innerHTML = "Register";
  ...
  new_xmr_estMaxAmount() { // may return null
    ...
    const availableWalletBalance = wallet
      .Balance_JSBigInt()
      .subtract(wallet.LockedBalance_JSBigInt()); // TODO: is it correct to incorporate locked balance into this?
    const estNetworkFee_moneroAmount = self.new_xmr_estFeeAmount();
    const possibleMax_moneroAmount = availableWalletBalance.subtract(
      estNetworkFee_moneroAmount
    );
    if (possibleMax_moneroAmount > 0) {
      // if the Max amount is greater than 0
  ...
  new_displayCcyFormatted_estMaxAmountString() {
    ...
    const xmr_estMaxAmount_str =
      beldex_amount_format_utils.formatMoney(xmr_estMaxAmount);
    //
    ...
    if (displayCcySymbol != Currencies.ccySymbolsByCcy.XMR) {
      ...
      const displayCurrencyAmountDouble_orNull =
        Currencies.displayUnitsRounded_amountInCurrency(
  ...
  _new_estimatedNetworkFee_displayString() {
    ...
    const estimatedTotalFee_str = beldex_amount_format_utils.formatMoney(
      estimatedTotalFee_JSBigInt
    );
    const estimatedTotalFee_moneroAmountDouble = parseFloat(
      estimatedTotalFee_str
    );
    ...
    let finalizable_formattedAmountString = estimatedTotalFee_str; // `${estimatedTotalFee_moneroAmountDouble}`
    {
      if (displayCcySymbol != Currencies.ccySymbolsByCcy.XMR) {
        ...
        if (displayCurrencyAmountDouble_orNull != null) {
          const displayCurrencyAmountDouble =
            displayCurrencyAmountDouble_orNull;
          finalizable_formattedAmountString =
            Currencies.nonAtomicCurrency_formattedString(
              displayCurrencyAmountDouble,
  ...
  set_isSubmittable_needsUpdate() {
    ...
    let isSubmittable =
      self.isFormDisabled != true && self.isResolvingSendTarget != true;
    const raw_amount_String = self.amountInputLayer.value;

    if (typeof raw_amount_String !== "undefined" && raw_amount_String) {
      const selected_ccySymbol =
        self.ccySelectLayer.Component_selected_ccySymbol();
      const rawInput_amount_Number = +raw_amount_String; // turns string into JS Number
      if (!isNaN(rawInput_amount_Number)) {
        const submittableMoneroAmountDouble_orNull =
          Currencies.submittableMoneroAmountDouble_orNull(
            self.context.CcyConversionRates_Controller_shared,
            selected_ccySymbol,
            rawInput_amount_Number
          );
        if (submittableMoneroAmountDouble_orNull == null) {
          // amt input exists but no converted amt found
          if (selected_ccySymbol == Currencies.ccySymbolsByCcy.XMR) {
            throw Error(
              "null submittableMoneroAmountDouble_orNull while selected_ccySymbol=.XMR"
            );
          }
  ...
  set_effectiveAmountLabelLayer_needsUpdate() {
    ...
    function __setTextOnAmountUI(title, shouldHide_tooltipButton) {
      ...
      if (shouldHide_tooltipButton != true) {
        ...
        self.effectiveAmountLabelLayer.appendChild(
          self.effectiveAmountLabel_tooltipLayer
        ); // NOTE: adding this /after/ updating title text
      }
      //
    ...
    if (selected_ccySymbol == XMR && displayCcySymbol == XMR) {
      // special case - no label necessary
      __hideEffectiveAmountUI();
      return;
    }
    //
    const xmrAmountDouble_orNull =
      Currencies.submittableMoneroAmountDouble_orNull(
        self.context.CcyConversionRates_Controller_shared,
        selected_ccySymbol,
        rawInput_amount_Number
    ...
    if (selected_ccySymbol == XMR) {
      ...
      const moneroAmountDouble_atomicPlaces =
        xmrAmountDouble * Math.pow(10, beldex_config.coinUnitPlaces);
      const moneroAmount = new JSBigInt(moneroAmountDouble_atomicPlaces);
      const formatted_moneroAmount =
        beldex_amount_format_utils.formatMoney(moneroAmount);
  ...
  refresh_networkFeeEstimateLayer() {
    const self = this;
    self.networkFeeEstimateLayer.innerHTML =
      self._new_estimatedNetworkFee_displayString();
  }
  ...
  _givenBootedSettingsController_setCcySelectLayer_initialValue() {
    ...
    if (self.context.settingsController.hasBooted != true) {
      throw Error(
        "_givenBootedSettingsController_setCcySelectLayer_initialValue called but !self.context.settingsController.hasBooted"
      );
    }
  ...
  _disable_submitButton() {
    const self = this;
    if (
      typeof self.rightBarButtonView !== "undefined" &&
  ...
  _displayResolvedAddress(address) {
    ...
    if (
      typeof self.resolvedAddress_containerLayer === "undefined" ||
      !self.resolvedAddress_containerLayer
    ) {
      throw Error(
        "_displayResolvedAddress expects a non-nil self.resolvedAddress_containerLayer"
      );
    }
  ...
  _displayResolvedPaymentID(payment_id) {
    ...
    if (
      typeof self.resolvedPaymentID_containerLayer === "undefined" ||
      !self.resolvedPaymentID_containerLayer
    ) {
      throw Error(
        "_displayResolvedPaymentID expects a non-nil self.resolvedPaymentID_containerLayer"
      );
    }
  ...
  //
  _tryToGenerateRegister(valuesArrayFromObject) {
    ...
    const wallet = self.walletSelectView.CurrentlySelectedRowItem;
    {
      if (typeof wallet === "undefined" || !wallet) {
        _trampolineToReturnWithValidationErrorString(
          "Please create a wallet to send Beldex."
        );
        return;
      }
    }
    ...
    // }
    //
    const hasPickedAContact = !!(
      typeof self.pickedContact !== "undefined" && self.pickedContact
    );
    ...
    const resolvedAddress_fieldIsVisible =
      self.resolvedAddress_containerLayer.style.display === "block";
    // Check if Yat
    if (self.isYat) {
    ...
    const manuallyEnteredPaymentID_fieldIsVisible =
      self.manualPaymentIDInputLayer_containerLayer.style.display === "block"; // kind of indirect, would be better to encapsulate show/hide & state, maybe
    ...
    const resolvedPaymentID_fieldIsVisible =
      self.resolvedPaymentID_containerLayer.style.display === "block";
    //
    ...
    if (canUseManualPaymentID && hasPickedAContact) {
      throw Error(
        "canUseManualPaymentID shouldn't be true at same time as hasPickedAContact"
      );
    ...
    const destinations = [
      {
        to_address:
          typeof enteredAddressValue !== "undefined"
            ? enteredAddressValue
            : resolvedAddress,
    ...
    if (!sweeping && selected_ccySymbol != Currencies.ccySymbolsByCcy.XMR) {
      const hasAgreedToUsageGateTerms =
        self.context.settingsController
      ...
      if (hasAgreedToUsageGateTerms == false) {
        ...
        self.context.windowDialogs.PresentQuestionAlertDialogWith(
          ...
          function (err, didChooseYes) {
            ...
            __proceedTo_generateRegisterTransaction(valuesArrayFromObject);
          }
        );
        //
        ...
        const message = `Register ${final_XMR_amount_Number} ${Currencies.ccySymbolsByCcy.XMR}?`;
        const ok_buttonTitle = "Register";
        const cancel_buttonTitle = "Cancel";
        self.context.windowDialogs.PresentQuestionAlertDialogWith(
          title,
          message,
    ...
    function __proceedTo_generateRegisterTransaction() {
      ...
      wallet.RegisterFunds(
        ...
        hasPickedAContact
          ? self.pickedContact.cached_OAResolved_XMR_address
          : undefined,
        hasPickedAContact
          ? self.pickedContact.HasOpenAliasAddress()
          : undefined,
        ...
        //
        function (
          str // preSuccess_nonTerminal_statusUpdate_fn
        ) {
        ...
        function (err, mockedTransaction) {
          ...
          }
          {
            ...
            const stateCachedTransaction =
              wallet.New_StateCachedTransaction(mockedTransaction); // for display
            self.pushDetailsViewFor_transaction(wallet, stateCachedTransaction);
          }
          {
            const this_pickedContact =
              hasPickedAContact == true ? self.pickedContact : null;
  ...
  pushDetailsViewFor_transaction(sentFrom_wallet, transaction) {
    ...
    if (
      ...
      throw Error(
        self.constructor.name + " requires navigationController to " + _cmd
      );
    }
    {
      const options = {
      ...
      const view = new JustRegisterTransactionDetailsView(
        options,
        self.context
      ); // note JustRegisterTransactionDetailsView
  ...
  viewWillAppear() {
    ...
    if (
      typeof self.navigationController !== "undefined" &&
      self.navigationController !== null
    ) {
      self.layer.style.paddingTop = `${self.navigationController.NavigationBarHeight()}px`;
      //
      self.qrCodeInputs_contentView.layer.style.height = `calc(100% - ${
        15 * 2 + self.navigationController.NavigationBarHeight() + 2
      }px)`; // +2 for border
      self.qrCodeInputs_contentView.layer.style.marginTop = `${
        15 + self.navigationController.NavigationBarHeight()
      }px`;
    }
  ...
  _didPickContact(contact) {
    ...
    }
    {
      ...
      if (self.pickedContact.HasOpenAliasAddress() === false) {
        ...
        // they're using an OA addr, so we still need to check if they still have one
        self._hideResolvedPaymentID(); // in case it's visible… although it wouldn't be
      }
    }
    {
    ...
    self.requestHandle_for_oaResolution =
      self.context.openAliasResolver.ResolveOpenAliasAddress(
        ...
        function (
          ...
          //
          if (
            typeof self.requestHandle_for_oaResolution === "undefined" ||
            !self.requestHandle_for_oaResolution
          ) {
            console.warn(
              "⚠️  Called back from ResolveOpenAliasAddress but no longer have a self.requestHandle_for_oaResolution. Canceled by someone else? Bailing after neutralizing UI."
            );
            return;
          }
          self.requestHandle_for_oaResolution = null;
          //
          if (
            typeof self.pickedContact === "undefined" ||
            !self.pickedContact
          ) {
            console.warn(
              "⚠️  Called back from ResolveOpenAliasAddress but no longer have a self.pickedContact. Bailing"
            );
            return;
          }
          if (self.pickedContact.address !== addressWhichWasPassedIn) {
            console.warn(
              "⚠️  The addressWhichWasPassedIn to the ResolveOpenAliasAddress call of which this is a callback is different than the currently selected self.pickedContact.address. Bailing"
            );
            return;
          }
          if (err) {
          ...
          {
            if (
              typeof moneroReady_address !== "undefined" &&
              moneroReady_address
            ) {
  ...
  _didFinishTypingInContactPickerInput(optl_event) {
    const self = this;
    //
    const enteredPossibleAddress =
      self.contactOrAddressPickerLayer.ContactPicker_inputLayer.value;
    const hasEnteredNoAddressContent =
      !enteredPossibleAddress || typeof enteredPossibleAddress === "undefined";
    //
    const wasEnterKey = optl_event
      ? optl_event.keyCode == 13
      : false; /* 'input' event which lacks 'e' arg in cb seems not to be called on 'enter' key */
    if (wasEnterKey) {
      const requestExists =
        typeof self.requestHandle_for_oaResolution !== "undefined" &&
        self.requestHandle_for_oaResolution !== null;
      if (requestExists) {
        // means we are currently requesting still and they just hit the enter btn - just "ignore"
        console.warn(
          "User hit return on contact picker input while still resolving a contact. Bailing."
        );
        return;
      }
    }
    ...
    let hasEmojiCharacters = /\p{Extended_Pictographic}/u.test(
      enteredPossibleAddress
    );
    if (hasEmojiCharacters) {
      let isYat = yatMoneroLookup.isValidYatHandle(enteredPossibleAddress);
      if (isYat) {
        const lookup = yatMoneroLookup
          .lookupMoneroAddresses(enteredPossibleAddress)
          .then((responseMap) => {
    ...
    if (isOAAddress !== true) {
      ...
      try {
        address__decode_result = self.context.monero_utils.decode_address(
          enteredPossibleAddress,
    ...
    if (wasEnterKey) {
      ...
      // To fill the UX gap, for now, on success of the below resolve, we call _tryToGenerateRegister
    }
    self.requestHandle_for_oaResolution =
      self.context.openAliasResolver.ResolveOpenAliasAddress(
        enteredPossibleAddress,
        function (
          ...
          if (enteredPossibleAddress !== addressWhichWasPassedIn) {
            console.warn(
              "⚠️  The addressWhichWasPassedIn to the ResolveOpenAliasAddress call of which this is a callback is different than the enteredPossibleAddress. Bailing"
            );
            return;
          }
          if (err) {
          ...
          if (typeof payment_id !== "undefined" && payment_id) {
            self.addPaymentIDButtonView.layer.style.display = "none";
            self.manualPaymentIDInputLayer_containerLayer.style.display =
              "none";
            self.manualPaymentIDInputLayer.value = "";
            self._displayResolvedPaymentID(payment_id);
          } else {
            ...
            if (
              ...
              // if manual payment field not showing
              self.addPaymentIDButtonView.layer.style.display = "block"; // then make sure we are at least shwign the + payment ID btn
            } else {
              // then one or the other is already visible - respect existing state
              console.log(
                "💬  It should be the case that either add pymt id btn or manual payment field is visible"
              );
            }
          }
          //
          if (wasEnterKey) {
            self._tryToGenerateRegister(); // to fulfil what the user is expecting this to do
          }
        }
      );
  }
  ...
  _shared_didPickQRCodeWithImageSrcValue(imageSrcValue) {
    ...
    img.addEventListener("load", function () {
      ...
      if (!code || !code.location) {
        self.validationMessageLayer.SetValidationError(
          "MyBeldex was unable to find a QR code in that image."
        );
        return;
      }
      ...
      if (!stringData) {
        self.validationMessageLayer.SetValidationError(
          "MyBeldex was unable to decode a QR code from that image."
        );
        return;
      }
      if (typeof stringData !== "string") {
        self.validationMessageLayer.SetValidationError(
          "MyBeldex was able to decode QR code but got unrecognized result."
        );
        return;
      }
      const possibleURIString = stringData;
      self._shared_didPickPossibleRequestURIStringForAutofill(
        possibleURIString
      );
    });
  ...
  _shared_didPickPossibleRequestURIStringForAutofill(possibleUriString) {
    ...
    try {
      requestPayload =
        monero_requestURI_utils.New_ParsedPayload_FromPossibleRequestURIString(
          possibleUriString,
          self.context.nettype,
      ...
      if (errStr) {
        self.validationMessageLayer.SetValidationError(
          "Unable to use the result of decoding that QR code: " + errStr
        );
        return;
      }
    }
    self._shared_havingClearedForm_didPickRequestPayloadForAutofill(
      requestPayload
    );
  }
  ...
  _shared_havingClearedForm_didPickRequestPayloadForAutofill(requestPayload) {
    const self = this;
    {
      ...
      if (
        amountCcy != null &&
        typeof amountCcy !== "undefined" &&
        amountCcy !== ""
      ) {
        if (
          self.amountInputLayer.value == null ||
          self.amountInputLayer.value == "" ||
          typeof self.amountInputLayer.value === "undefined" ||
          didSetAmountFromRequest
        ) {
          // so either the ccy and amount were on the request OR there was a ccy but the amount field was left empty by the user, i.e. we can assume it's ok to modify the ccy since there was one on the request
          self.ccySelectLayer.value = amountCcy;
        }
      } else {
    ...
    const payment_id_orNull =
      requestPayload.payment_id &&
      typeof requestPayload.payment_id !== "undefined"
        ? requestPayload.payment_id
        : null;
    if (
      typeof self.context.contactsListController !== "undefined" &&
      self.context.contactsListController != null &&
      self.context.contactsListController.records
    ) {
      if (
        target_address !== null &&
        ...
        for (let i = 0; i < numberOf_contacts; i++) {
          ...
          if (
            contact.address == target_address ||
            contact.cached_OAResolved_XMR_address == target_address
          ) {
        ...
        if (foundContact) {
          ...
          // we have an addr but no contact
          if (self.pickedContact && typeof self.pickedContact !== "undefined") {
            // unset
            self.contactOrAddressPickerLayer.ContactPicker_unpickExistingContact_andRedisplayPickInput(
              true // true, do not focus input
    ...
    }
    {
      ...
      if (payment_id_orNull !== null) {
        // but display it as a 'detected' pid
        ...
        if (
          typeof self.manualPaymentIDInputLayer.value === "undefined" ||
          !self.manualPaymentIDInputLayer.value
        ) {
  ...
  __shared_isAllowedToPerformDropOrURLOpeningOps() {
    const self = this;
    if (
      self.context.passwordController.HasUserEnteredValidPasswordYet() === false
    ) {
      return false;
    }
    ...
    if (
      !self.context.walletsListController.records ||
      self.context.walletsListController.records.length == 0
    ) {
      return false;
    }
    if (self.isFormDisabled === true) {
    ...
    if (self.navigationController.stackViews.length != 1) {
      // we will never see this case, because we auto-pop to root (self)
      // not going to return false here - they will be auto-dismissed and probably are still transitioning
    }
    return true;
  }
  ...
  _proxied_ondrop(e) {
    ...
    if (
      ...
      absoluteFilePath != "" &&
      typeof absoluteFilePath !== "undefined"
    ) {
      self._shared_didPickQRCodeAtPath(absoluteFilePath);
    } else if (file_size) {
      // going to assume we're in a browser
      if (!/^image\//.test(file.type)) {
        self.validationMessageLayer.SetValidationError(
          "Please select a QR code image file."
        );
        return;
      }
        ...
        reader.onload = (function (anImg) {
          const fn = function (e) {
            ...
            if (loadedBase64Content.indexOf("data:image/") != 0) {
              self.validationMessageLayer.SetValidationError(
                "Couldn't get QR code content from that file."
              );
              return;
            }
...
module.exports = RegisterFundsView;
